...
Run `npm run dev:server` in one terminal and `npm run dev` in another for online matches. The browser connects to `http://localhost:8000` by default; set `VITE_BGIO_SERVER=http://host:8000` for a remote lobby/game server. The server stores match data in `./storage` unless `BGIO_STORAGE_DIR` is set.
